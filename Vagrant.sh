...
if [ $(sudo -u postgres psql -tc "SELECT count(*) FROM pg_database WHERE datname='sistemico_development'") == 0 ]; then
  sudo -u postgres createdb -O vagrant sistemico_development
fi

if [ $(sudo -u postgres psql -tc "SELECT count(*) FROM pg_database WHERE datname='sistemico_test'") == 0 ]; then
  sudo -u postgres createdb -O vagrant sistemico_test
fi

if [ $(sudo -u postgres psql -tc "SELECT count(*) FROM pg_database WHERE datname='sistemico_production'") == 0 ]; then
  sudo -u postgres createdb -O vagrant sistemico_production
fi
...
development:
  <<: *default
  database: sistemico_development

test:
  <<: *default
  database: sistemico_test

production:
  <<: *default
  database: sistemico_production
EOF
  rake db:migrate
  rake db:setup
popd
